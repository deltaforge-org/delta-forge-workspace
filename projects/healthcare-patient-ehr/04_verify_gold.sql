...
ASSERT ROW_COUNT = 16
SELECT COUNT(*) AS row_count FROM {{zone_prefix}}.gold.dim_diagnosis;
...
SELECT COUNT(*) AS audit_entries FROM {{zone_prefix}}.silver.audit_log;
...
SELECT
    ...
    new_values,
    change_timestamp
FROM {{zone_prefix}}.silver.audit_log
