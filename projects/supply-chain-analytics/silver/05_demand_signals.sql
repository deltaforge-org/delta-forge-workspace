...
CREATE TABLE IF NOT EXISTS {{zone_prefix}}_silver.cleansed.demand_signals (
  store_id          STRING,
  sku               STRING,
  sale_date         DATE,
  qty_sold          INT,
...
MERGE INTO {{zone_prefix}}_silver.cleansed.demand_signals AS tgt
USING (
  ...
  store_warehouse AS (
    ...
    JOIN {{zone_prefix}}_bronze.raw.warehouses w ON s.region = w.region
  ),
  inventory AS (
