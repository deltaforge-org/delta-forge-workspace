...
OPTIMIZE {{zone_prefix}}_silver.cleansed.inventory_positions
  ZORDER BY (warehouse_id, sku);

OPTIMIZE {{zone_prefix}}_silver.cleansed.order_fulfillment
  ZORDER BY (supplier_id, sku, order_date);

OPTIMIZE {{zone_prefix}}_silver.cleansed.shipment_tracking
  ZORDER BY (shipment_id, current_status);

OPTIMIZE {{zone_prefix}}_silver.cleansed.demand_signals
  ZORDER BY (store_id, sku, sale_date);

-- Gold fact tables: optimize on common filter patterns
OPTIMIZE {{zone_prefix}}_gold.analytics.fact_inventory
  ZORDER BY (warehouse_id, sku, snapshot_date);

OPTIMIZE {{zone_prefix}}_gold.analytics.fact_orders
  ZORDER BY (supplier_id, sku, order_date);

-- Gold KPIs: optimize for dashboard queries
OPTIMIZE {{zone_prefix}}_gold.analytics.kpi_supplier_scorecard
  ZORDER BY (supplier_id);

OPTIMIZE {{zone_prefix}}_gold.analytics.kpi_inventory_health
  ZORDER BY (warehouse_id, sku, health_status);

OPTIMIZE {{zone_prefix}}_gold.analytics.kpi_demand_forecast
