...
CREATE OR REPLACE TABLE {{zone_prefix}}_gold.analytics.fact_orders AS
SELECT
  of.po_id,
  of.supplier_id,
  ds.supplier_name,
...
FROM {{zone_prefix}}_silver.cleansed.order_fulfillment of
JOIN {{zone_prefix}}_gold.analytics.dim_supplier ds ON ds.supplier_id = of.supplier_id
JOIN {{zone_prefix}}_gold.analytics.dim_product dp ON dp.sku = of.sku
LEFT JOIN (
  ...
  SELECT DISTINCT
    ...
    st.transit_hours,
    st.has_exception
  FROM {{zone_prefix}}_bronze.raw.warehouse_movements wm_receipt
  JOIN {{zone_prefix}}_bronze.raw.warehouse_movements wm_ship
  ...
  JOIN {{zone_prefix}}_silver.cleansed.shipment_tracking st
