...
SCHEDULE telecom_daily_schedule CRON '0 2 * * *' TIMEZONE 'UTC' RETRIES 3 TIMEOUT 3600 MAX_CONCURRENT 1 INACTIVE;

PIPELINE telecom_cdr_pipeline DESCRIPTION 'Daily CDR pipeline with schema evolution lifecycle, session reconstruction, and churn scoring' SCHEDULE 'telecom_daily_schedule' TAGS 'telecom,cdr,schema-evolution,churn,sessions' SLA 60 FAIL_FAST true LIFECYCLE production;
...
-- =============================================================================
-- STEP 1: Validate bronze tables across all 3 schema versions
-- =============================================================================
...
-- =============================================================================
-- STEP 2: Merge v1 records into unified silver (voice-only, NULL-fill new cols)
-- =============================================================================

STEP merge_schema_v1
...
-- =============================================================================
-- STEP 3: Merge v2 records into unified silver (multi-service, NULL-fill 5G cols)
-- =============================================================================

STEP merge_schema_v2
  DEPENDS ON (validate_bronze)
  TIMEOUT '5m'
AS
  MERGE INTO telco.silver.cdr_unified AS tgt
...
-- =============================================================================
-- STEP 4: Merge v3 records into unified silver (5G/roaming, full column set)
-- =============================================================================

STEP merge_schema_v3
  DEPENDS ON (validate_bronze)
  TIMEOUT '5m'
AS
  MERGE INTO telco.silver.cdr_unified AS tgt
...
-- =============================================================================
-- STEP 5: Apply type widening — duration INT -> BIGINT on unified table
-- =============================================================================
...
STEP apply_type_widening
  DEPENDS ON (merge_schema_v1, merge_schema_v2, merge_schema_v3)
  TIMEOUT '2m'
AS
  SELECT
      schema_version,
...
-- =============================================================================
-- STEP 6: Upsert subscriber profiles from unified CDR activity
-- =============================================================================

STEP upsert_subscribers
...
-- =============================================================================
-- STEP 7: Reconstruct sessions from CDR events using LAG gap detection
-- =============================================================================
...
-- =============================================================================
-- STEP 8: Build dim_tower
-- =============================================================================

STEP build_dim_tower
...
-- =============================================================================
-- STEP 9: Build dim_plan (distinct plan combinations)
-- =============================================================================
...
-- =============================================================================
-- STEP 10: Build dim_subscriber
-- =============================================================================

STEP build_dim_subscriber
  DEPENDS ON (upsert_subscribers, reconstruct_sessions)
  TIMEOUT '2m'
AS
...
-- =============================================================================
-- STEP 11: Build fact_calls (star schema)
-- =============================================================================

STEP build_fact_calls
...
-- =============================================================================
-- STEP 13: VACUUM and OPTIMIZE (maintenance — safe to fail)
-- =============================================================================

STEP vacuum_and_optimize
  DEPENDS ON (kpi_network_quality, kpi_churn_risk)
  CONTINUE ON FAILURE
  TIMEOUT '10m'
AS
