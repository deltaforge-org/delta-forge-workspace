...
ASSERT ROW_COUNT = 15
SELECT COUNT(*) AS row_count FROM {{zone_prefix}}.gold.dim_case;

ASSERT ROW_COUNT = 10
SELECT COUNT(*) AS row_count FROM {{zone_prefix}}.gold.dim_attorney;

ASSERT ROW_COUNT = 25
SELECT COUNT(*) AS row_count FROM {{zone_prefix}}.gold.dim_party;
...
FROM {{zone_prefix}}.gold.dim_attorney da
...
FROM {{zone_prefix}}.gold.dim_case dc
...
FROM {{zone_prefix}}.gold.kpi_firm_performance kfp
...
SELECT
    ...
    pp.cases_as_witness,
    pp.cases_as_expert
FROM {{zone_prefix}}.silver.party_profiles pp
