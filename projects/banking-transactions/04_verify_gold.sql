...
ASSERT ROW_COUNT = 12
SELECT COUNT(*) AS row_count FROM bank.gold.dim_account;

ASSERT ROW_COUNT = 15
SELECT COUNT(*) AS row_count FROM bank.gold.dim_merchant;

-- dim_date should cover Jan-Mar 2024 (91 days)
ASSERT VALUE date_dim_count >= 91
SELECT COUNT(*) AS date_dim_count FROM bank.gold.dim_date;
...
SELECT COUNT(*) AS distinct_days FROM bank.gold.kpi_daily_volumes;
...
SELECT MIN(running_7d_avg_txns) AS min_7d_avg FROM bank.gold.kpi_daily_volumes;

ASSERT VALUE max_7d_avg <= 20
SELECT MAX(running_7d_avg_txns) AS max_7d_avg FROM bank.gold.kpi_daily_volumes;
...
FROM bank.gold.fact_transactions f
LEFT JOIN bank.gold.dim_account da ON f.account_key = da.account_key
WHERE da.account_key IS NULL;

ASSERT VALUE orphan_dates = 0
SELECT COUNT(*) AS orphan_dates
FROM bank.gold.fact_transactions f
LEFT JOIN bank.gold.dim_date dd ON f.date_key = dd.date_key
...
FROM bank.silver.customer_dim VERSION AS OF 1;
