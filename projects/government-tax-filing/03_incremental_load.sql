...
PRINT {{INCREMENTAL_FILTER({{zone_prefix}}.silver.filings_immutable, filing_id, filing_date, 7)}};

-- ===================== SCHEDULE & PIPELINE =====================

SCHEDULE tax_incremental_schedule CRON '0 */6 * * *' TIMEZONE 'America/New_York' RETRIES 2 TIMEOUT 1800 MAX_CONCURRENT 1 ACTIVE;
...
AS
  ...
  WHERE {{INCREMENTAL_FILTER({{zone_prefix}}.silver.filings_immutable, filing_id, filing_date, 7)}};
...
AS
  ...
  WHERE fi.filing_date > (
      ...
      FROM {{zone_prefix}}.silver.audit_trail
...
AS
  ...
  USING (
      SELECT
          (SELECT COALESCE(MAX(filing_key), 0) FROM {{zone_prefix}}.gold.fact_filings)
...
STEP rebuild_kpi_preparer
  ...
  TIMEOUT '3m'
AS
  DELETE FROM {{zone_prefix}}.gold.kpi_preparer_quality WHERE 1=1;
