...
FROM {{zone_prefix}}.silver.sla_violations sv
JOIN {{zone_prefix}}.gold.dim_carrier dc ON sv.carrier_id = dc.carrier_key
JOIN {{zone_prefix}}.gold.dim_location ol ON sv.origin_id = ol.location_key
JOIN {{zone_prefix}}.gold.dim_location dl ON sv.destination_id = dl.location_key
...
FROM {{zone_prefix}}.gold.kpi_sla_compliance
...
FROM {{zone_prefix}}.gold.fact_shipments f
JOIN {{zone_prefix}}.gold.dim_carrier dc ON f.carrier_key = dc.carrier_key
JOIN {{zone_prefix}}.gold.dim_customer dcust ON f.customer_key = dcust.customer_key
...
FROM {{zone_prefix}}.gold.fact_shipments f
LEFT JOIN {{zone_prefix}}.gold.dim_location ol ON f.origin_key = ol.location_key
...
FROM {{zone_prefix}}.gold.fact_shipments f
LEFT JOIN {{zone_prefix}}.gold.dim_route dr ON f.route_key = dr.route_key
